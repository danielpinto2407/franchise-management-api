...
class ProductMongoAdapterTest {
        ...
        void shouldFindMaxStockProductByBranch() {
                ...
                franchise.setId("anyFranchise");
                franchise.setBranchIds(List.of("b1", "b2"));

                when(mongoTemplate.findOne(any(Query.class), eq(Franchise.class), eq("franchises")))
                                .thenReturn(Mono.just(franchise));

                when(mongoTemplate.find(any(Query.class), eq(Product.class)))
                                .thenAnswer(invocation -> {
                                        Query query = invocation.getArgument(0);
                                        branchId = query.getQueryObject().get("branchId").toString();
                                        if (branchId.equals("b1")) {
                                                return Flux.just(p2);
                                        } else if (branchId.equals("b2")) {
                                                return Flux.just(p4);
                                        } else {
                                                return Flux.empty();
                                        }
                                });
                when(mongoTemplate.find(argThat(query -> query.getQueryObject().get("branchId").equals("b2")),
                                eq(Product.class)))
                                .thenReturn(Flux.just(p4)); // Producto con mayor stock de b2
                ...
                StepVerifier.create(productMongoAdapter.findMaxStockProductByBranch("anyFranchise"))
                                ...
                                .consumeRecordedWith(products -> {
                                        ...
                                        assertTrue(productIds.containsAll(List.of("2", "4")) && productIds.size() == 2);
                                })
                                .verifyComplete();

                verify(mongoTemplate).findOne(any(Query.class), eq(Franchise.class), eq("franchises"));
                verify(mongoTemplate, times(2)).find(any(Query.class), eq(Product.class));
        }
