...
class UpdateProductStockUseCaseTest {
    ...
    void shouldUpdateProductStockSuccessfully() {
        ...
        StepVerifier.create(useCase.updateStock(franchiseId, branchId, productId, newStock))
        ...
        verify(productRepository, times(1))
                .updateProductStock(eq(franchiseId), eq(branchId), eq(productId), eq(newStock));
    }
    ...
    void shouldUpdateStockToZero() {
        ...
        StepVerifier.create(useCase.updateStock(franchiseId, branchId, productId, zeroStock))
    ...
    void shouldUpdateStockToLargeNumber() {
        ...
        StepVerifier.create(useCase.updateStock(franchiseId, branchId, productId, largeStock))
    ...
    void shouldHandleNegativeStockError() {
        ...
        StepVerifier.create(useCase.updateStock(franchiseId, branchId, productId, negativeStock))
                .expectErrorMatches(throwable -> throwable instanceof IllegalArgumentException &&
    ...
    void shouldHandleConcurrentUpdateError() {
        ...
        RuntimeException concurrentError = new RuntimeException("Product stock was modified by another process");
        ...
        StepVerifier.create(useCase.updateStock(franchiseId, branchId, productId, newStock))
                .expectErrorMatches(throwable -> throwable.getMessage().contains("modified by another process"))
                .verify();
    }
    ...
    void shouldUpdateStockMultipleTimes() {
        ...
        StepVerifier.create(useCase.updateStock(franchiseId, branchId, productId, firstStock))
                .expectNextMatches(p -> p.getStock().equals(firstStock))
                .verifyComplete();

        StepVerifier.create(useCase.updateStock(franchiseId, branchId, productId, secondStock))
